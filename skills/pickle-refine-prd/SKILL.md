...
**Default behavior (in-place refinement)**: Overwrite the **original input PRD file** (the exact path the user supplied to you in Step 1) with the fully refined version.
   ...
   - **THEATER AUDIT + PRE-EMIT GATES (non-negotiable before emit, per synthesis PRD `prds/claude-to-grok-ports-emission-quality-and-autonomous-reliability-2026-05-24.md`)**: For every proposed Verify in the AC/Verification tables (and Test Expectations), run the equivalent of `detectVerifyTheater` (from engine/src/lib/pipeline-preflight.ts) + mentally execute a BASELINE form on current tree. Enforce the new prescriptive Verify format (`— Verify: \`cmd\` — Type: ...`), Test Expectations table, forward-ref hygiene (exact `(forward-created)` / `(created by ticket ...)` annotation outside backticks, one space, pre-verified via git), and "no unresolved placeholders survive emission" rule. 
     ...
     - **Post-synthesis enforcement (manager must execute before calling emit or sealing)**:
       1. Parse every `## ac_shape_smells` JSON block from the three analysts' final round outputs (use regex or node -e to extract the object after the heading). Collect into a `smells` array (the real analyst data, per analyst-gate-injections + personas).
       2. The real preflight hygiene fns (`scanAnalystOutputsForUnverifiedPaths` + `checkVerifyMachinability` + now `runAcShapeEnforcement` from engine/src/lib/pipeline-preflight.ts) + `detectVerifyTheater` catch path/forward-ref violations, unverified backticks (R-RTRC-7 one-space + git proof), low-machinability prose, verify-theater patterns, **and AC-shape smells** (endpoint enumeration without universal quantifier / describe.each or missing // JUSTIFICATION: on splits). Run them on the combined analyst outputs + draft TicketSpec[] + manifest. (Ported verbatim from ../pickle-rick-claude/extension/src/bin/spawn-refinement-team.ts:1410 + check-readiness per 5+ swarm agents + 2026-05-24 PRD P0.)
       3. Any blocking errors from the real fns (including ac_shape violations from runAcShapeEnforcement returning non-0) are **hard** for council/meta paths: rewrite clean (collapse or add justification + parametrized test) **or** attach explicit sibling H-VERIFY-* healer(s) in the same emit batch **and** note in synthesis-enforcement-report.md. The emitter now calls the gate; no amber waiver for AC-shape on council/meta (hard block per claude main:1978 exit(2) before seal). Normal paths keep never-stop + healer promotion. Only clean or explicitly healed output reaches `emitRefineCouncilTickets`.
       Emit clear errors in your thinking + write a `synthesis-enforcement-report.md` under session with the violations (or note the attached healers). Only clean or explicitly healed output reaches `emitRefineCouncilTickets`. (Cross-checked vs Claude sibling check-readiness + forward-ref patterns + ac-shape-audit.)
...
const result = await emitRefineCouncilTickets(sessionDir, specs, {
  ...
  acShapeSmells: smells,   // NEW: real data flows to gate; absent/[] for non-council paths (no behavior change)
});
...
- **Post-synthesis emission quality gate + healer contract**: The unified preflight scans (machinability, path/forward-ref hygiene, AC-shape) + emitter's HARD EMISSION GATE are mandatory. For council/normal paths, theatrical/red findings trigger auto sibling H-VERIFY-EMIT-* healers + amber debt waiver on the affected tickets (see ticket-emitter.ts never-stop logic + research.md WAIVER section). The gate report + healers are *expected* artifacts for the closer/self-PRD; they do **not** prevent <promise>REFINEMENT_COMPLETE</promise> or auto-chain on council paths. Only meta/self-generated or unwaived poison hard-fails the refine manager. This is the designed "progress then fix" resilience.
- The improved runner skip behavior for research Verify blocks on EMISSION_THEATER / no runnable Verify (researcher emits full artifact + <promise> + RA blocked) with lack of evidence (no commits since pre-research set at boundary) are normal terminal `skipped` state for *that ticket*: auto `status: skipped` + skipReason (non-blocking for EPIC_COMPLETED / phase done / meta executable / pause). Other tickets proceed. Healing via H-VERIFY-* (manual add or closer/citadel-detected) or edit status to pending + resume. Never lets one early theatrical ticket freeze the autonomous campaign. See ritual.ts research rescue + orchestrator halt + phase-utils getReady + session markSkipped + research.md.
- Always emit the 4 proactive hardening tickets (code-qual, dataflow, test-qual, xref-consistency) for non-trivial cases + theater H-VERIFY healers (see Step 3). Hardening tickets are mandatory for any change that touches the meta surfaces (ritual, session, citadel, orchestrator, git_safety, self-*).
- Scope lists in tickets must be brutally honest — the ConvergenceGate will later punish violations.
- The original PRD (now updated in place) must be good enough that Citadel and the next self-PRD generator are happy with it. If a sidecar was explicitly requested, the `prd_refined.md` must also satisfy the same bar.
- **Post-synthesis emission quality gate + healer contract**: The unified preflight scans (machinability, path/forward-ref hygiene, AC-shape) + emitter's HARD EMISSION GATE are mandatory. For council/normal paths, theatrical/red findings trigger auto sibling H-VERIFY-EMIT-* healers + amber debt waiver on the affected tickets (see ticket-emitter.ts never-stop logic + research.md WAIVER section). The gate report + healers are *expected* artifacts for the closer/self-PRD; they do **not** prevent <promise>REFINEMENT_COMPLETE</promise> or auto-chain on council paths. Only meta/self-generated or unwaived poison hard-fails the refine manager. This is the designed "progress then fix" resilience.
