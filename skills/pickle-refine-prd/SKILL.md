...
**Default behavior (in-place refinement)**: Overwrite the **original input PRD file** (the exact path the user supplied to you in Step 1) with the fully refined version.
   ...
   - **THEATER AUDIT + PRE-EMIT GATES (non-negotiable before emit, per synthesis PRD `prds/claude-to-grok-ports-emission-quality-and-autonomous-reliability-2026-05-24.md`)**: For every proposed Verify in the AC/Verification tables (and Test Expectations), run the equivalent of `detectVerifyTheater` (from engine/src/lib/pipeline-preflight.ts) + mentally execute a BASELINE form on current tree. Enforce the new prescriptive Verify format (`— Verify: \`cmd\` — Type: ...`), Test Expectations table, forward-ref hygiene (exact `(forward-created)` / `(created by ticket ...)` annotation outside backticks, one space, pre-verified via git), and "no unresolved placeholders survive emission" rule. 
     ...
     - **Post-synthesis enforcement (manager must execute before calling emit or sealing)**:
       ...
       2. The real preflight hygiene fns (`scanAnalystOutputsForUnverifiedPaths` + `checkVerifyMachinability` from engine/src/lib/pipeline-preflight.ts) + `detectVerifyTheater` catch path/forward-ref violations, unverified backticks (R-RTRC-7 one-space + git proof), low-machinability prose, and verify-theater patterns. Run them on the combined analyst outputs + draft TicketSpec[] + manifest. `evaluateAcShapeEnforcement` (full machine AC-shape collapse-or-justify) is the remaining piece (see reliability-backlog + R10 fidelity audit 019e5a71-5ad9... + ../pickle-rick-claude spawn-refinement-team.ts:1410).
       3. Any blocking errors from the real fns, plus uncollapsed ac_shape smells without // JUSTIFICATION: + test, are severe: rewrite clean **or** attach explicit sibling H-VERIFY-* healer(s) in the same emit batch (the emitter auto-handles theater/red cases for council paths with amber + waiver per never-stop contract). Do not emit poison that would hard-stop the autonomous campaign on normal paths.
       Emit clear errors in your thinking + write a `synthesis-enforcement-report.md` under session with the violations (or note the attached healers). Only clean or explicitly healed output reaches `emitRefineCouncilTickets`. (Cross-checked vs Claude sibling check-readiness + forward-ref patterns.)
