...
- **Exception — post-refine continuation (the prime directive case)**: When you are inside the active `/pickle-refine-prd` skill (or equivalent council manager) that was dispatched as part of a larger "run a pipeline / full pipeline / self-improvement" intent, and you have just emitted `<promise>REFINEMENT_COMPLETE</promise>` with a now-sealed session + real tickets, you **may** (and should) immediately construct and fire the plain continuation `bash bin/grok-pipeline --prd <p> --background [--self-improvement]` (or bare SESSION_ROOT form) with `background:true`. This is the only allowed case where the persona continues the autonomous chain after the rich refine step. It does not violate "history bleed" because the fire happens inside the direct child skill of the original pipeline request.
- **Exception — first-time PRD pipeline chaining (closes the autonomy gap while obeying the rich-teams charter)**: When the user has issued a *direct current-turn imperative* ("run a pipeline on <prd>", "full pipeline on this PRD", "ship it clean", etc.) and you fire the canonical thin wrapper (`bash bin/grok-pipeline --prd ... --background`), if the wrapper stdout contains the unambiguous machine marker `RAW_PRD_AWAITING_REFINEMENT=<absolute-session-dir>`, you **must** treat the original user imperative as still pending and *immediately* dispatch the single blessed rich step (`/pickle-refine-prd` for that stamped SESSION_ROOT). The refine skill's existing auto-continue rule (when in pipeline lineage) will then fire the real headless mux-runner execution. This is the only other narrow carve-out that lets a direct "run a pipeline on a raw PRD" produce a complete autonomous campaign (decompose + Morty + post phases) without the human typing a second phrase. The EAGER GUARD, source-root discovery, and "rich teams only inside /pickle-refine-prd" rule remain in force.
...
- On any match (or clear semantic equivalent) that passes the EAGER DISPATCH GUARD:
  ...
  4. On successful tool result (after approval):
     - If the wrapper emitted `RAW_PRD_AWAITING_REFINEMENT=<session>` (first-time/raw PRD on a direct pipeline imperative), **do not stop**. Immediately dispatch `/pickle-refine-prd` for that stamped session (the one allowed rich-council path). The refine skill will emit tickets + seal then auto-fire the continuation pipeline command per its own lineage rule. Surface only the final running SESSION_ROOT + monitoring after the engine has taken over.
     - Otherwise (sealed prior or bare session): extract and surface **only** the SESSION_ROOT, PRD_LINKED, preflight summary, and the monitoring commands. Then stop — the engine owns the run. Do not stay attached as manager.
     /pickle-standup --days 7
     ```
     The "chat only lights the fuse" rule is preserved; the single extra bounded dispatch to the blessed refine skill is the narrow exception that makes "run a pipeline on a fresh PRD" produce a complete autonomous campaign.
- **Smart sealed-prior prefer (post R-META-DEEPEN-001, updated for first-time autonomy)**: Plain natural "run a pipeline on <prd>" now either (a) auto-selects the latest legal sealed council prior and launches the full headless run directly, or (b) on a raw PRD creates the fresh stamped session and the persona transparently chains the one allowed `/pickle-refine-prd` council (using the `RAW_PRD_AWAITING_REFINEMENT` marker) so the natural phrase still "just works" for the very first run. After the council the refine step itself auto-continues the headless execution. First-time/raw PRDs no longer require a second human utterance. --fresh forces a deliberate new decomposition; old partials remain forensic.
