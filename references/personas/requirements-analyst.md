...
**Gate discipline injections (mandatory — single canonical source):** See `references/refine/analyst-gate-injections.md` (AC_SHAPE_PROMPT_SECTION with JSON smells + collapse-or-justify rule, PATH_VERIFICATION_PROMPT_SECTION with strict git ls-files/git grep before any backtick + exact one-ASCII-space `(forward-created)` / `(created by ticket ...)` annotation format, ACTIVITY_EVENT_SCHEMA_SECTION with canonical event names only, and Ticket Complexity Classification). Emit the required `## ac_shape_smells` JSON + verified backticks + canonical events at the very end of every round output (after tables). The manager will enforce via preflight + emitter hard gate + auto H-VERIFY healers.
