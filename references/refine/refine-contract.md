...
- **Post-synthesis emission quality gate** (engine/src/lib/pipeline-preflight.ts) — the unified home (after deletion of the parallel readiness-gate). Enforces machinability (`checkVerifyMachinability` + `MACHINE_HINT_RE` vs `PURE_PROSE_RE`), path/symbol hygiene with exact one-space forward-ref annotation + git verification (`scanAnalystOutputsForUnverifiedPaths`), AC-shape smells (`evaluateAcShapeEnforcement`), and theater detection (`detectVerifyTheater`). Wired into `ticket-emitter.ts` HARD EMISSION GATE (auto H-VERIFY sibling + amber waiver for council paths per never-stop contract) + SKILL synthesis step. Failures on meta/self paths are hard; council debt produces healers + report artifacts for closer/self-PRD. See the 2026-05-24 synthesis PRD (full port from Claude sibling's check-readiness + forward-ref service).
