...
- **Post-synthesis emission quality gate** (engine/src/lib/pipeline-preflight.ts) — the unified home (after deletion of the parallel readiness-gate). Enforces machinability (`checkVerifyMachinability`), path/symbol hygiene with exact one-ASCII-space forward-ref annotation + git verification (`scanAnalystOutputsForUnverifiedPaths`), theater detection (`detectVerifyTheater`), and (via auto-attached H-VERIFY-EMISSION-HONESTY sibling + manual review per injected sections) AC-shape smells. Full `evaluateAcShapeEnforcement` fn is the remaining piece (reliability-backlog; see 2026-05-24 PRD + R10 fidelity audit). Wired into `ticket-emitter.ts` HARD EMISSION GATE (auto H-VERIFY sibling + amber waiver for council paths per never-stop contract) + SKILL synthesis step. Failures on meta/self paths are hard; council debt produces healers + report artifacts for closer/self-PRD. Cross-checked vs Claude sibling (check-readiness.ts + spawn-refinement-team.ts:1410/1925 + forward-ref-annotation).
