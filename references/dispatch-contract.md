...
- The full **EAGER DISPATCH GUARD** (verbatim or equivalent) to prevent accidental runs on hedged/quoted/internal text, wrong --target (source vs deployed P0), and manager resurrection. Plain --prd dispatches now auto-prefer a legal sealed prior session when one exists, **or** (for a raw PRD) the persona chains the single allowed `/pickle-refine-prd` council via the `RAW_PRD_AWAITING_REFINEMENT` marker so a direct "run a pipeline on <any prd>" still produces a complete autonomous run on the first utterance.
