...
**Last Updated**: 2026-05-27 (creation tranche)
...
- engine/src/self-prd-generator.ts:335-336: fidelityDirs includes 'docs'; fidelityKeywords matches "closer-ticket-manager-handoff". If found in real docs/ and !backlog.closedCategories.has('self-loop-ingestion') → GAP-SELF-LOOP-INGESTION (P0) at 358-364.
- engine/src/self-prd-generator.ts:723-726: listed in performPostCampaignIngest candidates. If exists + /PASS|closed|resilience|ritual|persistence|meta/i.test → closed++ + "Ingested ..." line (729-736).
...
- engine/tests/self-prd-closer.test.ts:176-178: test *creates stub* in tmp/docs/ precisely to exercise the self-loop ingestion gap path and prove generator surfaces it (167-198 full test).
...
- All cross-refs use exact line# (AGENTS:40, reliability-backlog:53, install.sh:26/63, generator:725, PRD:69, test:178).
...
## Current Status (Post-Creation)

- Debt closed for this item: real living doc now at docs/closer-ticket-manager-handoff.md.
- Still dominant open (AGENTS:38): full ac_shape_smells JSON plumbing + richer annotation_format in manifest, living MASTER_PLAN updates depth, thin citadel depth.
