...
- `engine/src/lib/pipeline-preflight.ts` (the canonical post-synthesis/pre-emit emission quality gate after unification). Exports `detectVerifyTheater`, `scanAnalystOutputsForUnverifiedPaths`, `checkVerifyMachinability`, `evaluateAcShapeEnforcement`, `assessMetaReadiness`, etc. + R-RTRC-7 path/symbol hygiene (exact one-space forward-ref annotations) + machinability/contract. Wired into `ticket-emitter.ts` (HARD EMISSION GATE + auto H-VERIFY healers), refine SKILL synthesis enforcement, and run-pipeline preflight. Failures surface via amber + sibling healers (never-stop) or hard block on meta/self paths. See the 2026-05-24 synthesis PRD for the full port from the Claude sibling.
