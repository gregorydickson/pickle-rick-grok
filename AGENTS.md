...
- **No overclaims. Ever.** 
  ...
  - 50-ticket overnight self-run + full meta self-PRD loop: **GO**. Fire detached (`mux-runner` or `pipeline --self-improvement --target . --no-refine`), walk away, check delta in morning. Resumable, atomic, resource-bounded, citadel-gated, backlog-shrinking. Discover root is now deterministic (always lands on the `pickle-rick-grok/` tree even from monorepo parent). (Provenance seal now resilient for --no-refine re-dispatch on both human and R-META self-PRDs via merge write + getManifestPrdPath canonical extra; see pipeline-preflight.ts contract. Backend Reviewer-Fixer 2026-05-20: robust corrupt-sidecar rescue + full-state ids + richer ILLEGAL RCA diags/report.)
...
- **Pipeline Never Modifies Deployed pickle-rick-grok Code — Absolute, Non-Negotiable Invariant**
  ...
  - Enforcement lives in: `git_safety.ts`, `resource-guard.ts`, `session.ts` (workingDirSafe), `workers.ts` (workingDir passed to exec), `orchestrator.ts`, `ritual.ts`, `arch-deepener.ts` (FORBIDDEN_SELF_MUT + isSelfMutationSafe + filterSelfMut), and every `writeJsonAtomic` / file-write site. All of them are required to resolve and stay inside the source root.
...
- [2026-05-19] arch-deepener self-mut: explicit FORBIDDEN_SELF_MUT + filterSelfMut in scanForOpportunities + applyChange guard + exported isSelfMutationSafe predicate. Closes unconditional emission of core drivers (iteration/ritual/gate/session/workers/arch-deepener) + self-deepening opp on any engine/src target (P0-SELF-MUT-001 + H-003). Scanner + Citadel self-meta auditor now share the canonical list.
